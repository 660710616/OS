...
public class Client {
    ...
    public Client() {
        try {
            ...
            //ข้อมูลจะถูก “flush” ออกไปทันที ไม่ค้างใน Buffer ถ้าไม่ใส่ true โปรแกรมอาจไม่ส่งข้อมูลจนกว่าจะ flush เองหรือปิด stream
            this.inByteFromServer = new BufferedInputStream(clientSocket.getInputStream());
            //clientSocket.getInputStream() คือ ช่องทางรับข้อมูลจาก server เป็น stream แบบ byte 
            ...
            while (!clientSocket.isClosed()) {
                ...
                clientCommand = sc.nextLine();// รับค่าจาก client  //nextLine() อ่านค่าที่ผู้ใช้พิมพ์เข้ามาทั้งบรรทัดรวมช่องว่างด้วย
                ...
                if (clientCommand.equals("f")) {
                    ...
                    outputToServer.println(type);// ส่ง type ไป server หลังจากนั้น server จะส่งข้อมูลกลับมาเป็น tyepe ข้อมูลกับ ข้อมูล

                    String messageFromServer = inputFromServer.readLine(); // receive Long/String from server อ่านข้อมูลบรรทัดแรกที่ server ส่งมา
    ...
    public void copy(String fileName, long fileSize) {
        ...
        try {
            ...
            byte[] buffer = new byte[1024];// กำหนดขนาด buffer สำหรับที่พักการอ่านข้อมูลชั่วคราว(RAM) โปรแกรมจะอ่านข้อมูลจาก server ทีละก้อน (chunk) ขนาด 1 KB ไม่กิน RAM มากเกินไป
            int bytesRead;
            ...
            while (totalBytesRead < fileSize && (bytesRead = inByteFromServer.read(buffer)) != -1) {
                ...
                //inByteFromServer.read(buffer) อ่านข้อมูลจาก server ใส่ลงใน buffer ถ้าไม่มีแล้ว จะคืนค่า -1 แล้วหยุด loop
            ...
            if (totalBytesRead != fileSize) {//เช็คว่าอ่านข้อมูลครบตามขนาดไฟล์ที่ server ส่งมาไหม
                System.err
                        .println("Warning: File size mismatch. Expected: " + fileSize + ", but got: " + totalBytesRead);//err ขึ้นเตือนเป็นสีแดง
            } else {
    ...
    public void zeroCopy(String fileName, long fileSize) {//ระบบปฏิบัติการ(OS) ใช้ buffer ภายในเคอร์เนล(kernel buffer) ให้โดยอัตโนมัติ ใช้ direct memory access (DMA) ช่วยในการโอนถ่ายข้อมูลระหว่างอุปกรณ์เครือข่ายกับหน่วยความจำหลักโดยไม่ต้องผ่าน CPU
        //ส่งข้อมูลจาก SocketChannel → FileChannel โดยตรง
        try (FileChannel destinationFile = new FileOutputStream(clientPath + "Zerocopy-" + fileName).getChannel()) {//สร้าง FileChannel สำหรับเขียนไฟล์ปลายทาง
            ...
            long position = 0;// ตำแหน่งเริ่มต้นในการเขียนไฟล์ ทำให้รู้ว่าตอนนี้เราอ่านหรือส่งข้อมูลไปถึงตรงไหนแล้ว
            long count; // จำนวน byte ที่ถูกโอนถ่ายในแต่ละครั้ง

            System.out.println("Start downloading file...");
            while (position < fileSize && (count = destinationFile.transferFrom(clientChannel, position, fileSize - position)) > 0) {
                //ส่งข้อมูลจากไฟล์ sourceFileเริ่มตั้งแต่ byte ที่ position ส่งไปยัง clientChannel โดยส่งได้มากที่สุดเท่ากับ (ขนาดไฟล์ทั้งหมด - ตำแหน่งปัจจุบัน)
                position += count;
            }
